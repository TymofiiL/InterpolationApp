...
Завантажити останній реліз без збірки з вихідного коду:
[**Releases →**](https://github.com/TymofiiL/InterpolationApp/releases/latest)

| Платформа | Файл                          | Запуск                                                                                      |
| --------- | ----------------------------- | ------------------------------------------------------------------------------------------- |
| Windows   | `InterpolationApp-windows.zip` | Розпакувати → `InterpolationApp.exe`                                                        |
| macOS     | `InterpolationApp-macos.zip`  | Розпакувати → правою кнопкою на `.app` → **Відкрити** → **Відкрити** (лише перший раз) |

> Застосунок для macOS не підписаний сертифікатом Apple Developer, тому Gatekeeper блокує
> перший запуск подвійним кліком. Правий клік → **Відкрити** вирішує це раз і назавжди.
> Або в терміналі: `xattr -cr /шлях/до/InterpolationApp.app`
...
- Введення вузлів (xᵢ, yᵢ) у таблиці — додавання, редагування, видалення рядків (до 10 000 вузлів)
- Завантаження вузлів із файлу TXT (до 10 000 вузлів; роздільники полів: пробіл, табуляція, крапка з комою)
...
Роздільники полів: пробіл, табуляція, крапка з комою (і кома як запасний варіант).  
Десятковий роздільник: крапка або кома (`0,5` і `0.5` — еквівалентні).  
Максимум: **10 000 вузлів** на файл.
